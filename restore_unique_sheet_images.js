...
// Assign exact unique downloaded local image /perfumes/perfume_X.jpg to each product
const updatedProducts = productsSeed.map((p, index) => {
  const localImg = `/perfumes/perfume_${index + 1}.jpg`;
  return {
    ...p,
...
// 1. Save server/products_seed.json
fs.writeFileSync(seedPath, JSON.stringify(updatedProducts, null, 2));

// 2. Update src/data/initialProducts.ts
...
console.log('Successfully assigned 212 UNIQUE images to all 212 perfumes!');
